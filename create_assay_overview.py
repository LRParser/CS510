...
def main():
                ...
                for filename in filenames:
                    ...
                    for cid, outcome in zip(cid_series,outcome_series):
                        ...
                        try :
                            ...
                            #if outcome:
                            #    outcomes_kept.append(1)
                            #else:
                            #    outcomes_kept.append(0)

                            #cur.execute("SELECT * FROM pubchem where PUBCHEM_CID={0}".format(cid))
                            #rows = cur.fetchall()
                        ...
                        j = j + 1
                        if(j % 10000 == 0) :
                            print("Processed {0}".format(j))

                    s_molids = pd.Series(cids_kept, dtype="int64")
                    s_outcomes = pd.Series(outcomes_kept, dtype=bool)

                    parsed_df = pd.DataFrame({"mol_id" : s_molids, assay_name : s_outcomes})
                    #parsed_df =
                    #parsed_df.compute()
                    if full_df is None:
                        full_df = dd.from_pandas(parsed_df,npartitions=20)
                    else :
                        full_df = dd.merge(full_df,parsed_df,on='mol_id')
                        #full_df = full_df.merge(parsed_df, left_index=True, right_index=True)
                    full_df.compute()
    ...
    full_df.to_csv("/media/data/pubchem/assay_final.csv",index=False)
